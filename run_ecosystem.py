...
BIRD_TYPE_COLORS = {
    "default": (144,238,144), # 淡綠色
    "speed": (60, 179, 113),     # 春天綠
    "marathon": (0, 128, 0),  # 純綠
    "landmine": (65, 105, 225),  # 皇軍藍
...
def run_pygame(Setting, stop_event=None, shared_state_modify=None, shared_state_read=None,OLD_data=None,save_OLD_data=None,file_Record=None):
    ...
    class Bird(Animal):
        ...
        def apply_force(self, boids, Target):
            ...
            for i in Target:
                other = boids[i]
                #確保不是自己且對象是存活的
                if other is not self and other.situation == "alive" and other.Attribute["type"] == self.Attribute["type"]:
                    #計算兩個 boid 之間的距離
    ...
    class Predator(Animal):
        def __init__(self, pos = None, load_data=None, color = (255, 30, 45)):
            self.consume_time = 0
            self.Attribute = Setting["Predator"].copy()
            if load_data:
        ...
        def consume_bird(self, bird):
            ...
            self.consume_time = bird.size/8*self.Attribute["Consume_Time"]
            # self.color = (154,28,8)
    ...
    if OLD_data:
        birds = []
        for b_data in OLD_data["birds"]:
            b = Bird(load_data=b_data)
            bird_attr = b_data.get("Attribute", {})
            b_type = bird_attr.get("type","default")
            
            # 根據 type 設定顏色方便辨識
            b.color = BIRD_TYPE_COLORS.get(b_type)
            birds.append(b)
        predators = []
        for p_data in OLD_data["predators"]:
            # 取得這隻掠食者的類型 (例如 "speed")
            p_type = p_data.get("type", "default") # 預設為 default
            # 從 Setting 中抓取該類型的屬性設定
            type_setting = Setting["Predator"].get(p_type)
            
            # 建立掠食者個體
            # 我們傳入物理資料 (p_data) 與 屬性資料 (type_setting)
            p = Predator(load_data=p_data)
            
            # 4. 強制覆蓋屬性 (根據你的 Setting 檔內容)
            p.Attribute["Size"] = type_setting.get("Size")
            p.Attribute["MIN_Speed"] = type_setting.get("MIN_Speed")
            p.Attribute["MAX_Speed"] = type_setting.get("MAX_Speed")
            p.Attribute["Perception_Radius"] = type_setting.get("Perception_Radius")
            p.Attribute["Separation_Weight"] = type_setting.get("Separation_Weight")
            p.Attribute["Track_Weight"] = type_setting.get("Track_Weight")
            p.Attribute["Eat_Radius"] = type_setting.get("Eat_Radius")
            p.Attribute["Track_Mode"] = type_setting.get("Track_Mode")
            p.Attribute["Consume_Time"] = type_setting.get("Consume_Time")
            p.color = PREDATOR_TYPE_COLORS.get(p_type)
            p.size = p.Attribute["Size"]
            predators.append(p)
    ...
    while Running and (not stop_event or (stop_event and not stop_event.is_set())):
        ...
        for i in range(0,len(birds)):
            ...
            if (birds[i].situation == "dead"):
                ...
                # else: birds[i] = Bird()
                current_attribute = birds[i].Attribute.copy()
                b_type = current_attribute.get("type", "default")
                current_color = birds[i].color # 保留原本代表環境的顏色
                # 2. 建立一個全新的個體 (這會重置它的物理狀態，如位置)
                birds[i] = Bird() 
                # 3. 把備份的屬性塞回去，並填滿耐力
                birds[i].Attribute = current_attribute
                birds[i].Attribute["Survival_Time"] = 0
                birds[i].Attribute["Fitness"] = 0
                birds[i].color = BIRD_TYPE_COLORS.get(b_type)
                birds[i].stamina = current_attribute["MAX_Stamina"]
                birds[i].situation = "alive"
            else:
...
if __name__ == "__main__":
    ...
    run_pygame(Pygame_Setting,OLD_data=Pygame_OLD)